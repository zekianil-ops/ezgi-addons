...
<cfform name="quality_form" id="quality_form" method="post" action="">
    ...
    <table width="100%">
        <tr>
            <td>
				...
				<div class="col col-6 col-md-6">
					<div class="ui-card" <cfif not ListFind(x_video_control,get_operation_type.OPERATION_TYPE_ID)>style="display:none"</cfif>>
						<div class="ui-card-item">
    ...
    <cf_popup_box_footer>
        <table style="width:100%">
            <tr>
            	<td style="width:25%; text-align:left;<cfif not ListFind(x_qualite_control_confirmation,get_operation_type.OPERATION_TYPE_ID)>style="display:none"</cfif>>
            		<cf_get_lang dictionary_id='43704.Kalite Kontrol Sonuçları'>
              	</td>
            	<td style="width:25%; text-align:right; display:none">
		...
		if(type==1)
		{
			if(list_len(document.getElementById('quality_type_id_list').value))
			{
				...
				if(sor==true)
				{
					...
					var url = '<cfoutput>#request.self#?fuseaction=production.addoperationresult_ezgi&operation_type_id=#get_operation_type.OPERATION_TYPE_ID#&x_qualite_control_confirmation=#x_qualite_control_confirmation#&x_video_control=#x_video_control#&x_photo_control=#x_photo_control#&quality_control_process_stage=#get_process_type.PROCESS_ROW_ID#&trace_no=#attributes.trace_no#</cfoutput>&upd_id='+document.getElementById('upd_id').value+'&operation_id_='+document.getElementById('operation_id_').value+'&station_id_='+document.getElementById('station_id_').value+'&realized_amount_='+document.getElementById('realized_amount_').value+'&employee_id_='+document.getElementById('employee_id_').value+'&operation_gurup_id='+document.getElementById('operation_gurup_id').value+'&process_stage='+document.getElementById('process_stage').value+'&process_cat='+document.getElementById('process_cat').value+'&success_id_='+document.getElementById('success_id').value+'&convert_list_='+convert_list+'&quality=1';
