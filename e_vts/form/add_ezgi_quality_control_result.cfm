...
<cf_xml_page_edit>
	...
	<style>
	   ...
	   .fade:hover {opacity: 0.5;}
		input[id="pictures"], input[id="videos"] { display: none; }
		.custom-file-upload {
		...
		.imgPanel img, .videoPanel video{width: 100%; height: calc(100% - 10px);}
		.imgButton{width: 100%; padding: 5px;}
		.imgButton a{color: red;}
	</style>
	...
	<cfform name="quality_form" id="quality_form" method="post" action="">
			...
			<tr>
				<td>
						...
						<cf_ajax_list>
							<thead>
								<tr>
									...
									<th align="center" width="30%">&nbsp;<cf_get_lang dictionary_id="43705.Kalite Kontrol Kategorisi"></th>
									<th align="center" width="65%">&nbsp;<cf_get_lang dictionary_id="41580.Sonuç"></th>
								</tr>
							</thead>
							<tbody>
										...
										<td>
											<select id="quality_control_row_id_#GET_QUALITY_ROW_group.QUALITY_TYPE_ID#" name="quality_control_row_id_#GET_QUALITY_ROW_group.QUALITY_TYPE_ID#" style="height:38px; width:100%">
												<cfloop query="GET_QUALITY_ROW_detail">
													<option value="#QUALITY_CONTROL_ROW_ID#">#QUALITY_CONTROL_ROW#<cfif len(QUALITY_ROW_DESCRIPTION)>-#QUALITY_ROW_DESCRIPTION#</cfif></option>
												</cfloop>
											</select> 
										</td>
									</tr>
					...
					<div class="col col-6 col-md-6">
						<div class="ui-card" <cfif not ListFind(x_photo_control,get_operation_type.OPERATION_TYPE_ID)>style="display:none"</cfif>>
							<div class="ui-card-item">
								<label for="pictures" class="custom-file-upload">
									<i class="fa fa-camera"></i> Fotoğraf Çek
								</label>
								<input type="file" name="pictures" id="pictures" accept="image/*" capture="user" multiple onchange="MediaObject().photo.takePhoto()"/>
							</div>
							<div class="ui-card-item" id="photo-panel" style="display:none;"></div>
						</div>
					</div>
					<div class="col col-6 col-md-6">
						<div class="ui-card" <cfif not ListFind(x_photo_control,get_operation_type.OPERATION_TYPE_ID)>style="display:none"</cfif>>
							<div class="ui-card-item">
								<label for="videos" class="custom-file-upload">
									<i class="fa fa-video-camera"></i> Video Çek
								</label>
								<input type="file" name="videos" id="videos" accept="video/*" capture="environment" multiple onchange="MediaObject().video.takeVideo()"/>
							</div>
							<div class="ui-card-item" id="video-panel" style="display:none;"></div>
						</div>
					</div>
				</td>
			</tr>
		...
		<cf_popup_box_footer>
			<table style="width:100%">
				<tr>
					<td colspan="1" style="width:25%; text-align:left;<cfif not ListFind(x_qualite_control_confirmation,get_operation_type.OPERATION_TYPE_ID)>display:none</cfif>">
						<strong><cf_get_lang dictionary_id='57629.Açıklama'></strong>
					</td>
					<td colspan="2" style="width:70%; text-align:left;<cfif not ListFind(x_qualite_control_confirmation,get_operation_type.OPERATION_TYPE_ID)>display:none</cfif>">
						<input type="text" id="qualite_control_detail" name="qualite_control_detail" maxlength="100" style="width:99%; height:35px"> 
					</td>
				</tr>
				<tr>
					<td style="width:25%; text-align:left;<cfif not ListFind(x_qualite_control_confirmation,get_operation_type.OPERATION_TYPE_ID)>display:none</cfif>">
						<strong><cf_get_lang dictionary_id='43704.Kalite Kontrol Sonuçları'></strong>
					</td>
					<td style="width:25%; text-align:right;<cfif not ListFind(x_qualite_control_confirmation,get_operation_type.OPERATION_TYPE_ID)>display:none</cfif>">
						<select id="success_id" name="success_id#" style="height:38px; width:100%">
							  <cfoutput query="get_succsess">
								<option value="#success_id#,#IS_SUCCESS_TYPE#">#success#</option>
							</cfoutput>
						</select> 
					</td>
	...
	<script type="text/javascript">
		<cfif ListFind(x_photo_control,get_operation_type.OPERATION_TYPE_ID)>
			let allPhotos = [], 
				allVideos = [];
			/*Photo / Video Render*/
			const MediaObject = function () {
				return{
					photo: {
						takePhoto: function() {
							var photos = document.querySelector('#pictures').files;
							if (photos) {
								Array.from(photos).forEach(file => { allPhotos.push(file); });
								MediaObject().photo.addToLayer();
							}
						},
						addToLayer: function() {
							$("div#photo-panel").html("").show();
				
							allPhotos.forEach((el, index) => {
		
								var reader  = new FileReader();
								reader.addEventListener("load", function () {
									$("<div>").addClass("imgPanel").append(
										$("<img>").attr({"src": reader.result}),
										$("<div>").addClass("imgButton").append(
											$("<a>").attr({"href":"javascript://", "onclick": "MediaObject().photo.remove("+ index +")"}).text("Kaldır")
										)
									).appendTo($("div#photo-panel"));
								}, false);
						...
						remove: function (index) {
							...
							if(allPhotos.length > 0) MediaObject().photo.addToLayer();
							else $("div#photo-panel").html("").hide();
						}
					},
					video:{
						takeVideo: function () {
							...
							if (videos) {
								Array.from(videos).forEach(file => { allVideos.push(file); });
								MediaObject().video.addToLayer();
							}
						},
						addToLayer: function () {
							$("div#video-panel").html("").show();
							...
							allVideos.forEach((el, index) => {
								...
								reader.addEventListener("load", function (e) {
									...
									$("<div>").addClass("videoPanel").append(
										$("<video>").attr({"controls":""}).append(
											$("<source>").attr({"src":videourl})
										),
										$("<div>").addClass("imgButton").append(
											$("<a>").attr({"href":"javascript://", "onclick": "MediaObject().video.remove("+ index +")"}).text("Kaldır")
										)
									).appendTo($("div#video-panel"));
						...
						remove: function (index) {
							...
							if(allVideos.length > 0) MediaObject().video.addToLayer();
							else $("div#video-panel").html("").hide();
						}
					}
				}
			}
			...
			if(type==1)
			{
				<cfif ListFind(x_photo_control,get_operation_type.OPERATION_TYPE_ID)>
					if(list_len(document.getElementById('quality_type_id_list').value))
					{
						...
						if(sor==true)
						{
							...
							var url = '<cfoutput>#request.self#?fuseaction=production.addoperationresult_ezgi&quality=1&media_material=1&operation_type_id=#get_operation_type.OPERATION_TYPE_ID#&x_qualite_control_confirmation=#x_qualite_control_confirmation#&quality_control_process_stage=#get_process_type.PROCESS_ROW_ID#&trace_no=#attributes.trace_no#</cfoutput>&upd_id='+document.getElementById('upd_id').value+'&operation_id_='+document.getElementById('operation_id_').value+'&station_id_='+document.getElementById('station_id_').value+'&realized_amount_='+document.getElementById('realized_amount_').value+'&employee_id_='+document.getElementById('employee_id_').value+'&operation_gurup_id='+document.getElementById('operation_gurup_id').value+'&process_stage='+document.getElementById('process_stage').value+'&process_cat='+document.getElementById('process_cat').value+'&success_id_='+list_getat(document.getElementById('success_id').value,1)+'&convert_list_='+convert_list+'&quality_sucsess='+list_getat(document.getElementById('success_id').value,2)+'&qualite_control_detail='+document.getElementById('qualite_control_detail').value;
				...
				<cfelse>
					...
					if(sor==true)
					{
						...
						convert_list = convert_list.substr(0,convert_list.length-1);
						window.location='<cfoutput>#request.self#?fuseaction=production.addoperationresult_ezgi&operation_type_id=#get_operation_type.OPERATION_TYPE_ID#&x_qualite_control_confirmation=#x_qualite_control_confirmation#&quality_control_process_stage=#get_process_type.PROCESS_ROW_ID#&trace_no=#attributes.trace_no#</cfoutput>&upd_id='+document.getElementById('upd_id').value+'&operation_id_='+document.getElementById('operation_id_').value+'&station_id_='+document.getElementById('station_id_').value+'&realized_amount_='+document.getElementById('realized_amount_').value+'&employee_id_='+document.getElementById('employee_id_').value+'&operation_gurup_id='+document.getElementById('operation_gurup_id').value+'&process_stage='+document.getElementById('process_stage').value+'&process_cat='+document.getElementById('process_cat').value+'&success_id_='+list_getat(document.getElementById('success_id').value,1)+'&convert_list_='+convert_list+'&quality=1&quality_sucsess='+list_getat(document.getElementById('success_id').value,2)+'&qualite_control_detail='+document.getElementById('qualite_control_detail').value;
					}
					else
						return false;
				</cfif>
