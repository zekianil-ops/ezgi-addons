...
<cf_workcube_process 
            ...
            record_date='#now()#' 
            data_source='#dsn3#'
            action_page='#request.self#?fuseaction=prod.list_results&event=upd&p_order_id=#attributes.upd_id#&pr_order_id=#ADD_PRODUCTION_ORDER.MAX_ID#' 
            action_id='#ADD_PRODUCTION_ORDER.MAX_ID#'
            action_table='PRODUCTION_ORDER_RESULTS'
            action_column='PR_ORDER_ID'
            old_process_line='#ADD_PRODUCTION_ORDER.MAX_ID#' 
            warning_description = 'Üretim Sonucu : #paper_full#'>
<!--- Ezgi Bilgisayar Özelleştirme Bitişi --->
