...
<cfset savedPredictQuestionMap = {}>
<cfset hasImportRowIdForPredict = false>
<cfset importRowIdForPredict = 0>
<cfif not isdefined('is_form_submitted') and isdefined('attributes.ezgi_id') and val(attributes.ezgi_id) gt 0>
	<cftry>
		<cfquery name="get_import_row_for_predict" datasource="#dsn3#">
			SELECT TOP 1
				ISNULL(VIRTUAL_OFFER_IMPORT_ROW_ID,0) AS VIRTUAL_OFFER_IMPORT_ROW_ID
			FROM
				EZGI_VIRTUAL_OFFER_ROW
			WHERE
				EZGI_ID = <cfqueryparam value="#attributes.ezgi_id#" cfsqltype="cf_sql_numeric">
		</cfquery>
		<cfif get_import_row_for_predict.recordcount>
			<cfset importRowIdForPredict = val(get_import_row_for_predict.VIRTUAL_OFFER_IMPORT_ROW_ID)>
			<cfif importRowIdForPredict gt 0>
				<cfset hasImportRowIdForPredict = true>
			</cfif>
		</cfif>
		<cfcatch>
			<cfset hasImportRowIdForPredict = false>
...
<cfif hasImportRowIdForPredict>
	<cfquery name="get_saved_predict_values" datasource="#dsn3#">
		SELECT
			D.QUESTION_ID,
		...
		WHERE
			P.VIRTUAL_OFFER_IMPORT_ROW_ID = <cfqueryparam value="#importRowIdForPredict#" cfsqltype="cf_sql_numeric">
			AND D.IS_SELECTED = 1
	</cfquery>
	<cfloop query="get_saved_predict_values">
		<cfset savedPredictQuestionMap[trim(get_saved_predict_values.QUESTION_ID & "")] = {
			option_id = trim(get_saved_predict_values.ALTERNATIVE_STOCK_ID & ""),
			option_name = trim(get_saved_predict_values.ALTERNATIVE_VALUE & ""),
			match_status = trim(get_saved_predict_values.MATCH_STATUS & "")
		}>
	</cfloop>
</cfif>
<cfif not get_ROW.recordcount>
    ...
    <cfif not isdefined('is_form_submitted') and structCount(savedPredictQuestionMap)>
    	<cfoutput query="get_row">
        	<cfset predictionQuestionKey = trim(get_row.QUESTION_ID & "")>
            <cfif len(predictionQuestionKey) and structKeyExists(savedPredictQuestionMap, predictionQuestionKey)>
            	<cfif get_row.PIECE_TYPE eq 4>
                	<cfset 'attributes.alternative_stock_id_4_#get_row.PIECE_ROW_ID#' = savedPredictQuestionMap[predictionQuestionKey].option_id>
                <cfelse>
                	<cfset 'attributes.alternative_stock_id_#get_row.PIECE_TYPE#_#get_row.PIECE_ROW_ID#' = savedPredictQuestionMap[predictionQuestionKey].option_id>
                </cfif>
            </cfif>
        </cfoutput>
    </cfif>
                                 		...
                                 		<cfif get_product.is_prototip eq 1>
                                     		<cfif get_row.recordcount>
                                                ...
                                                <span id="guncelle_" <cfif not isdefined('attributes.is_form_submitted')>style="display:none"</cfif>>
													...
													<cfelse>
														<cfif Listlen(maillist) gt 0>
															<br>
															<span style="color:orange; font-weight:bold; text-align:center;">DİKKAT! Revizyon Talebi</span>
															<br><br>
															<cf_workcube_buttons is_upd='1' is_delete='0' is_cancel='1' add_function='control(3)'>
														</cfif>
													</cfif>
                                             	</span>
                                        	</cfif>
...
<script type="text/javascript">
	...
	function control(type)
	{
		...
		if(type == 3)
		{
			sor=confirm('İlgili Personele Revizyon Talebi Gönderilecektir. Devam etmek istiyor musunuz?');
			if(sor==true)
			{
				document.getElementById("addSpecAll").action = "<cfoutput>#request.self#?fuseaction=prod.emptypopup_upd_ezgi_virtual_offer_row_spect_aktar&revision=1&mail_list=#maillist#</cfoutput>";
				document.getElementById("addSpecAll").submit();
				return true;
			}
			else
	...
	function syncAlternativeAnswers(sourceElement)
	{
		...
		for(i = 0; i < questionSelects.length; i++)
		{
			...
			if(questionSelects[i].value && questionSelects[i].value != '0')
				continue;
