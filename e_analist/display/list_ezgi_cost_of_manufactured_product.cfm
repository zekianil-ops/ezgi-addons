...
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
		...
		<cfform name="form" action="#request.self#?fuseaction=account.list_ezgi_cost_of_manufactured_product" method="post">
			...
			<cf_box_search>
				<div class="form-group">
					<cfinput type="text" style="width:100px;" placeholder="Filtre" name="keyword" value="#attributes.keyword#">
				</div>
				...
				<div class="form-group">
					...
					<cfinput type="text" name="stock_name" id="stock_name" style="width:150px;" placeholder="Ürün" value="#attributes.stock_name#" onFocus="AutoComplete_Create('stock_name','PRODUCT_NAME','PRODUCT_NAME,STOCK_CODE','get_product_autocomplete','0','STOCK_ID,PRODUCT_ID','stock_id,product_id','','3','225');" autocomplete="off">
					<a href="javascript://" onClick="windowopen('<cfoutput>#request.self#</cfoutput>?fuseaction=objects.popup_product_names&field_id=form.stock_id&product_id=form.product_id&field_name=form.stock_name','list');"><img src="/images/plus_thin.gif" align="absmiddle" border="0"></a>
				</div>
				<div class="form-group">
					<input type="hidden" name="spect_main_id" value="<cfoutput>#attributes.spect_main_id#</cfoutput>">
					<cfinput type="text" name="spect_name" style="width:120px;" placeholder="Spec" value="#attributes.spect_name#">
				...
				<div class="form-group">
					<cf_wrk_search_button search_function='tarih_kontrolu()' button_type="4">
				</div>
			</cf_box_search>
	...
	<cfsavecontent variable="title">Üretilen Malın Maliyeti</cfsavecontent>
	<cf_box title="#title#" scroll="1">
		<cf_grid_list>
			<cfif isdefined("attributes.form_is_submitted") AND len(attributes.form_is_submitted)>
				...
				<cfif GET_PRODUCT_COST_ID.recordcount>
					<cfif gizle_id eq 1>
						<thead>
							<tr>
								...
								<cfif attributes.dsp_source eq 1>
									<th colspan="6" style="text-align:center">Kaynak Bilgileri</th>
								</cfif>
							</tr>
							<tr>
						...
						<cfquery name="GET_PRODUCT_COST_GROUP" dbtype="query">
							SELECT PR_ORDER_ID, COUNT(*) AS SAYI FROM GET_PRODUCT_COST_ID GROUP BY PR_ORDER_ID
						</cfquery>
						...
						<cfquery name="GET_PRODUCT_COST_GROUP_STOCK" dbtype="query">
							SELECT PR_ORDER_ID, SUM(AMOUNT_2) AS AMOUNT_2 FROM GET_PRODUCT_COST_ID WHERE NAME_PRODUCT_2 NOT LIKE 'Fason%' GROUP BY PR_ORDER_ID
						</cfquery>
							...
							<cfif P_ID neq 0>
								...
								<cfif GET_PRODUCT_COST.recordcount>
									...
									<cfquery name="PROCESS_TYPE_NAME" datasource="#dsn3#">
										SELECT PROCESS_CAT FROM SETUP_PROCESS_CAT WHERE PROCESS_CAT_ID = #GET_PRODUCT_COST.ACTION_PROCESS_CAT_ID#
									</cfquery>
					...
					<cfelse>
						<!--- Stok Bazında Görünüm --->
						<cfquery name="get_product_cost_group" dbtype="query">
							SELECT DISTINCT STOCK_CODE, NAME_PRODUCT, STOCK_ID, PRODUCT_ID, P_ORDER_ID, STATION_ID, PR_ORDER_ID, FINISH_DATE, RESULT_NO, SPECT_NAME, SPEC_MAIN_ID, SPECT_ID, AMOUNT, PRODUCT_COST_PURCHASE_NET_SYSTEM, PRODUCT_COST_PURCHASE_EXTRA_COST_SYSTEM, PRODUCT_COST_SYSTEM, PRODUCT_COST_PURCHASE_NET, PRODUCT_COST_PURCHASE_EXTRA_COST, PRODUCT_COST, STATION_REFLECTION_COST_SYSTEM, LABOR_COST_SYSTEM FROM GET_PRODUCT_COST_ID
						</cfquery>
						<cfquery name="get_product_cost_group_1" dbtype="query">
							SELECT COUNT(*) AS A_ROW, STOCK_CODE, NAME_PRODUCT, STOCK_ID, PRODUCT_ID, SUM(AMOUNT) AS AMOUNT, SUM(PRODUCT_COST_PURCHASE_NET_SYSTEM) AS PRODUCT_COST_PURCHASE_NET_SYSTEM, SUM(PRODUCT_COST_PURCHASE_EXTRA_COST_SYSTEM) AS PRODUCT_COST_PURCHASE_EXTRA_COST_SYSTEM, SUM(PRODUCT_COST_SYSTEM * AMOUNT) AS PRODUCT_COST_SYSTEM, SUM(PRODUCT_COST_PURCHASE_NET) AS PRODUCT_COST_PURCHASE_NET, SUM(PRODUCT_COST_PURCHASE_EXTRA_COST) AS PRODUCT_COST_PURCHASE_EXTRA_COST, SUM(PRODUCT_COST) AS PRODUCT_COST, SUM(STATION_REFLECTION_COST_SYSTEM) AS STATION_REFLECTION_COST_SYSTEM, SUM(LABOR_COST_SYSTEM) AS LABOR_COST_SYSTEM FROM get_product_cost_group GROUP BY STOCK_CODE, NAME_PRODUCT, STOCK_ID, PRODUCT_ID ORDER BY NAME_PRODUCT
						</cfquery>
						...
						<cfquery name="get_product_cost_group_2" dbtype="query">
							SELECT COUNT(*) AS A2_ROW, STOCK_ID, STOCK_CODE_2, NAME_PRODUCT_2, STOCK_ID_2, PRODUCT_ID_2, SUM(AMOUNT_2) AS AMOUNT_2, SUM(PRODUCT_COST_PURCHASE_NET_SYSTEM_2) AS PRODUCT_COST_PURCHASE_NET_SYSTEM_2, SUM(PRODUCT_COST_PURCHASE_EXTRA_COST_SYSTEM_2) AS PRODUCT_COST_PURCHASE_EXTRA_COST_SYSTEM_2, SUM(PRODUCT_COST_SYSTEM_2 * AMOUNT_2) AS PRODUCT_COST_SYSTEM_2, SUM(PRODUCT_COST_PURCHASE_NET_2) AS PRODUCT_COST_PURCHASE_NET_2, SUM(PRODUCT_COST_PURCHASE_EXTRA_COST_2) AS PRODUCT_COST_PURCHASE_EXTRA_COST_2, SUM(PRODUCT_COST_2) AS PRODUCT_COST_2, SUM(STATION_REFLECTION_COST_SYSTEM_2) AS STATION_REFLECTION_COST_SYSTEM_2, SUM(LABOR_COST_SYSTEM_2) AS LABOR_COST_SYSTEM_2 FROM GET_PRODUCT_COST_ID GROUP BY STOCK_ID, STOCK_CODE_2, NAME_PRODUCT_2, STOCK_ID_2, PRODUCT_ID_2
						</cfquery>
						<cfquery name="GET_PRODUCT_COST_GROUP_3" dbtype="query">
							SELECT STOCK_ID, COUNT(*) AS SAYI FROM get_product_cost_group_2 GROUP BY STOCK_ID
						</cfquery>
						<cfloop query="GET_PRODUCT_COST_GROUP_3">
							<cfset 'satir#STOCK_ID#' = GET_PRODUCT_COST_GROUP_3.SAYI>
						</cfloop>
						<cfquery name="GET_PRODUCT_COST_GROUP_STOCK" dbtype="query">
							SELECT STOCK_ID, SUM(AMOUNT_2) AS AMOUNT2_ FROM get_product_cost_group_2 WHERE NAME_PRODUCT_2 NOT LIKE 'Fason%' GROUP BY STOCK_ID
						</cfquery>
						<cfloop query="GET_PRODUCT_COST_GROUP_STOCK">
						...
						<thead>
							...
							<tr>
								...
								<th style="text-align:center; width:70px">Toplam Maliyet</th>
								<th style="text-align:center; width:70px">Tutar</th>
								<th style="text-align:center; width:70px" colspan="3">Fire</th>
								<th style="text-align:center; width:80px">Stok Kodu</th>
								<th style="text-align:center; width:100px">Ürün</th>
								<th style="text-align:center; width:70px">Miktar</th>
								<th style="text-align:center; width:70px">Net Maliyet</th>
								<th style="text-align:center; width:70px">Ek Maliyet</th>
								<th style="text-align:center; width:70px">Toplam Maliyet</th>
								<th style="text-align:center; width:70px" colspan="2">Tutar</th>
							</tr>
						</thead>
						<cfset k = 1>
						<tbody>
							<cfoutput query="get_product_cost_group_2">
								<tr>
									...
									<cfif k eq 1>
										...
										<td rowspan="#evaluate('satir#STOCK_ID#')#" style="text-align:right">#TlFormat(evaluate('PRODUCT_COST_PURCHASE_NET_SYSTEM#STOCK_ID#')/evaluate('A_ROW#STOCK_ID#'),4)#</td>
										<td rowspan="#evaluate('satir#STOCK_ID#')#" style="text-align:right">#TlFormat(evaluate('PRODUCT_COST_PURCHASE_EXTRA_COST_SYSTEM#STOCK_ID#')/evaluate('A_ROW#STOCK_ID#'),4)#</td>
										<td rowspan="#evaluate('satir#STOCK_ID#')#" style="text-align:right">#TlFormat(evaluate('PRODUCT_COST_SYSTEM#STOCK_ID#')/evaluate('A_ROW#STOCK_ID#'),4)#</td>
										<td rowspan="#evaluate('satir#STOCK_ID#')#" style="text-align:right">#TlFormat(evaluate('PRODUCT_COST_SYSTEM#STOCK_ID#'),2)#</td>
				...
				<cfelse>
					<tbody>
						<tr>
							<td colspan="23" style="text-align:center"><cf_get_lang dictionary_id='57484.Kayıt Yok'>!</td>
						</tr>
					</tbody>
				</cfif>
			<cfelse>
				<tbody>
					<tr>
						<td colspan="23" style="text-align:center"><cf_get_lang dictionary_id='57701.Filtre Ediniz'>!</td>
					</tr>
				</tbody>
			</cfif>
		</cf_grid_list>
