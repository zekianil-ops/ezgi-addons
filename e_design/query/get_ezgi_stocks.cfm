      	...
      	FROM
			...
			<cfif attributes.product_type eq 2>
                ...
                FROM            
                    ...
                    EZGI_DESIGN AS ED WITH (NOLOCK) ON EDMR.DESIGN_ID = ED.DESIGN_ID INNER JOIN
                    PRODUCT_CAT AS PC WITH (NOLOCK) ON ED.PRODUCT_CATID = PC.PRODUCT_CATID
                WHERE     
                    1=1   
           	...
           	<cfelseif attributes.product_type eq 3>
				...
				FROM            
                	EZGI_DESIGN AS ED WITH (NOLOCK) INNER JOIN
                 	PRODUCT_CAT AS PC WITH (NOLOCK) ON ED.PRODUCT_CATID = PC.PRODUCT_CATID INNER JOIN
                  	EZGI_DESIGN_PACKAGE_ROW AS EDPR WITH (NOLOCK) ON ED.DESIGN_ID = EDPR.DESIGN_ID
				WHERE        
                	ED.STATUS = 1
           	...
           	<cfelseif attributes.product_type eq 4>
				...
				FROM            
                	EZGI_DESIGN AS ED WITH (NOLOCK) INNER JOIN
                 	PRODUCT_CAT AS PC WITH (NOLOCK) ON ED.PRODUCT_CATID = PC.PRODUCT_CATID INNER JOIN
                 	EZGI_DESIGN_PIECE AS EDP WITH (NOLOCK) ON ED.DESIGN_ID = EDP.DESIGN_ID INNER JOIN
...
<cfelseif isdefined('attributes.ezgi_production') and isdefined('attributes.product_type') and (attributes.product_type eq 2 or attributes.product_type eq 3 or attributes.product_type eq 4)> <!---I Flow Üretim Sayfasından Geliyorsa--->
	<cfquery name="PRODUCTS" datasource="#DSN3#">
    	SELECT
			*
      	FROM
			...
			<cfif attributes.product_type eq 2>
				...
				FROM            
                	EZGI_DESIGN_MAIN_ROW AS EDMR WITH (NOLOCK) INNER JOIN
                  	EZGI_DESIGN AS ED WITH (NOLOCK) ON EDMR.DESIGN_ID = ED.DESIGN_ID INNER JOIN
                    #dsn1_alias#.PRODUCT_CAT AS PC WITH (NOLOCK) ON ED.PRODUCT_CATID = PC.PRODUCT_CATID INNER JOIN
                 	STOCKS AS S WITH (NOLOCK) ON EDMR.DESIGN_MAIN_RELATED_ID = S.STOCK_ID INNER JOIN
                   	#dsn1_alias#.PRODUCT_UNIT AS PU WITH (NOLOCK) ON S.PRODUCT_ID = PU.PRODUCT_ID
				WHERE  
           	...
           	<cfelseif attributes.product_type eq 3>
            	SELECT        
            		...
            		(SELECT SP.SPECT_MAIN_NAME FROM SPECT_MAIN SP WHERE SP.STOCK_ID = S.STOCK_ID AND SP.SPECT_MAIN_ID = (SELECT MAX(SMM.SPECT_MAIN_ID) FROM SPECT_MAIN SMM WHERE SMM.STOCK_ID = S.STOCK_ID AND SMM.IS_TREE = 1)) SPECT_MAIN_NAME
				FROM           
                	#dsn1_alias#.PRODUCT_UNIT AS PU WITH (NOLOCK) INNER JOIN
               		STOCKS AS S WITH (NOLOCK) ON PU.PRODUCT_ID = S.PRODUCT_ID INNER JOIN
                   	EZGI_DESIGN_PACKAGE_ROW AS EDP WITH (NOLOCK) ON S.STOCK_ID = EDP.PACKAGE_RELATED_ID INNER JOIN
                 	EZGI_DESIGN AS ED WITH (NOLOCK) INNER JOIN
                	#dsn1_alias#.PRODUCT_CAT AS PC WITH (NOLOCK) ON ED.PRODUCT_CATID = PC.PRODUCT_CATID ON EDP.DESIGN_ID = ED.DESIGN_ID
				WHERE 
                	S.PRODUCT_STATUS = 1 AND     
                	ED.STATUS = 1 AND 
                    PU.IS_MAIN = 1
            ...
            (SELECT SP.SPECT_MAIN_NAME FROM SPECT_MAIN SP WHERE SP.STOCK_ID = S.STOCK_ID AND SP.SPECT_MAIN_ID = (SELECT MAX(SMM.SPECT_MAIN_ID) FROM SPECT_MAIN SMM WHERE SMM.STOCK_ID = S.STOCK_ID AND SMM.IS_TREE = 1)) SPECT_MAIN_NAME
				FROM            
                  	...
                  	EZGI_DESIGN AS ED WITH (NOLOCK) INNER JOIN
                 	PRODUCT_CAT AS PC WITH (NOLOCK) ON ED.PRODUCT_CATID = PC.PRODUCT_CATID ON EDP.DESIGN_ID = ED.DESIGN_ID INNER JOIN
                  	#dsn1_alias#.PRODUCT_UNIT AS PU WITH (NOLOCK) INNER JOIN
                 	#dsn3_alias#.STOCKS AS S WITH (NOLOCK) ON PU.PRODUCT_ID = S.PRODUCT_ID ON EDP.PIECE_RELATED_ID = S.STOCK_ID
				WHERE    
                	ED.STATUS = 1 AND 
                    PU.IS_MAIN = 1 AND 
                    EDP.PIECE_TYPE <> 4 AND 
                    EDP.PIECE_STATUS = 1AND
                    S.PRODUCT_STATUS = 1
           	</cfif>
            ) AS TBL
     	WHERE
...
<cfelse>
    <cfquery name="PRODUCTS" datasource="#DSN3#">
        ...
        </cfif>---> <!---Tasarımda Sorun oluşturuyordu yoruma aldım---> 
        <cfif isDefined("attributes.keyword") and len(attributes.keyword)>
            AND 
            (
            <cfif len(attributes.keyword) eq 1 >
