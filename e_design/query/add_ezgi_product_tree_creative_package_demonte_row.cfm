                	...
                	<cfif get_pieces.PIECE_TYPE neq 4> <!---Parça Hammadde değil ise--->
                        ...
                        <cfif get_aks_row_info.recordcount>
                       		<cfloop query="get_aks_row_info">
                                ...
                                <cfif len(get_aks_row_info.MATERIAL_MEASURE1)>
                                    <cfset rawmaterial_detail = '#get_aks_row_info.MATERIAL_MEASURE1#X'>
                                </cfif>
                                <cfif len(get_aks_row_info.MATERIAL_MEASURE2)>
                                    <cfset rawmaterial_detail = '#rawmaterial_detail##get_aks_row_info.MATERIAL_MEASURE2#X'>
                                </cfif>
                                <cfif len(get_aks_row_info.MATERIAL_AMOUNT)>
                                    <cfset rawmaterial_detail = '#rawmaterial_detail# - #MATERIAL_AMOUNT# Adet'>
                                </cfif>
                            	<cfset 'attributes.piece_detail#currentrow#' = rawmaterial_detail>
