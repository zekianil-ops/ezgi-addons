<!---
    File: add_ezgi_palet_from_shelf_to_shelf.cfm
    Folder: Add_Ons\ezgi\e-pda\form
    Author: Ezgi Yazılım
    Date: 01/01/2007
    Description: Palet - Raf Değiştir
--->
...
<cfform name="form_basket">
	<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <cfsavecontent variable="title"><cf_get_lang dictionary_id="1286.Palet-Raf Değiştir"></cfsavecontent>
...
<script language="javascript" type="text/javascript">
	...
	function get_stock(barcode) /*Ürün Kontrolü*/
    {
		...
		var get_store_amount = wrk_safe_query('get_packing_barcode_ezgi','dsn3',0,listParam);
		...
		if(get_shelf.recordcount)
		{
			...
			else /* Form Giriş Depo ile Rafın Bulunduğu Lokasyon Eşit ise*/
			{
				if (document.getElementById('add_other_barcod').value.length > 0)
				{
					...
					{
						/*var palet_content_sql = "SELECT S.STOCK_ID,S.PRODUCT_NAME FROM STOCKS AS S INNER JOIN EZGI_PACKING_ROW AS EPR ON S.STOCK_ID = EPR.STOCK_ID INNER JOIN EZGI_PACKING AS EP ON EPR.PACKING_ID = EP.PACKING_ID WHERE EP.BARCODE = '"+document.getElementById('add_other_barcod').value+"'";*/ /*Paletin içerik Listesi Alınıyor*/
						/*var get_palet_content = wrk_query(palet_content_sql,'dsn3');*/
	...
	function add_row()
	{
		...
		other_shelf_code = document.getElementById('add_in_shelf').value;
		...
		document.getElementById('add_other_barcod').disabled = false;
		document.getElementById('add_out_shelf').disabled = false;
		document.getElementById('add_in_shelf').disabled = false;
		document.getElementById('add_other_barcod').value = '';
		document.getElementById('add_out_shelf').value = '';
		document.getElementById('add_in_shelf').value = '';
		document.getElementById('add_other_amount').value = 1;
	...
	function add_amount()
	{
		...
		row_count = document.getElementById('row_count').value;	
	  	if(row_count >0) /*ilk Satırdan sonrası*/
	  	{
		  	...
		  	{
				if(document.getElementById('paletid'+i).value == paletid)
				{
					if(document.getElementById('paletid'+i).value == paletid && document.getElementById('shelf_code_in'+i).value == other_shelf_code && document.getElementById('shelf_code_out'+i).value == shelf_code)
					{
		...
		if(row_count >0) /*ilk Satırdan sonrası*/
	  	{
			...
			else
			{
				document.getElementById('onay').disabled = true;
				actionidolustur();
				window.location.href='<cfoutput>#request.self#</cfoutput>?fuseaction=pda.add_ambar_fis&ambarfis=11&change_shelf_fis=1&dep_in='+form_basket.txt_department_in.value+'&dep_out='+form_basket.txt_department_out.value+'&palet_action_id='+document.getElementById('palet_action_id').value+'&fis_tipi='+form_basket.fis_tipi.value+'&process_cat='+form_basket.process_cat_id.value;
			}
		}
		else
		{
	...
	function actionidolustur()
	{
	  	...
	  	for(i=1;i<=row_count;i++)
	  	{
		  	if(document.getElementById('amount'+i).value > 0)
		  	{
				...
				document.getElementById('palet_action_id').value = document.getElementById('palet_action_id').value + document.getElementById('shelf_code_in'+i).value + '-';
