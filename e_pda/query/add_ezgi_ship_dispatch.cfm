...
<cfif ListLen(current_row_list)>
	<cfquery name="get_paper" datasource="#dsn3#">
    	SELECT SHIP_NO, SHIP_NUMBER FROM PAPERS_NO WHERE EMPLOYEE_ID = #SESSION.EP.USERID#
    </cfquery>
    ...
    <cfset attributes.DEPARTMENT_ID = Listgetat(attributes.dep_out,1,'-')>
    <cfset attributes.LOCATION_ID = Listgetat(attributes.dep_out,2,'-')> 
    <cfset attributes.ACTIVE_PERIOD = session.ep.period_id>
    <cfif isdefined('attributes.dep_in')>
    	<cfset attributes.DEPARTMENT_IN_ID = Listgetat(attributes.dep_in,1,'-')>
		<cfset attributes.LOCATION_IN_ID = Listgetat(attributes.dep_in,2,'-')> 
    <cfelse>
    ...
    <cfif not isdefined('attributes.ref_no')>
    ...
    <cfloop list="#current_row_list#" index="k">
        ...
        <cfif isdefined('attributes.dispatch') and attributes.dispatch eq 2> <!---İç Talep Bazlı Sevk İrsaliyesi İse--->
        	...
        	<cfset 'attributes.ROW_SHIP_ID#k#' = '1;#GET_EZGI_INTERNAL_ROW.I_ROW_ID#'>
        ...
        <cfset attributes.webService = 0>
    </cfloop>
</cfif>
<cfset form.process_cat = attributes.process_cat>	
<cfinclude template="add_ezgi_ship_dispatch_include.cfm">
<cfif isdefined('attributes.dispatch') and attributes.dispatch eq 2><!--- İç Talep Bazlı Sevk İrsaliyesi--->
	<script type="text/javascript">
		alert('Kayıt İşlemi Tamamlanmıştır.');
		windowopen('<cfoutput>#request.self#?fuseaction=objects.popup_print_files&action=stock.add_ship_dispatch&action_id=#MAX_ID.IDENTITYCOL#&print_type=30</cfoutput>','small');
		window.location ="<cfoutput>#request.self#?fuseaction=pda.add_ezgi_ship_dispatch_2</cfoutput>";
	</script>
	<!---<cflocation url="#request.self#?fuseaction=pda.add_ezgi_ship_dispatch_2" addtoken="No">--->
<cfelse><!--- Depolararası Sevk İrsaliyesi--->
	<cflocation url="#request.self#?fuseaction=pda.add_ezgi_ship_dispatch" addtoken="No">
</cfif>