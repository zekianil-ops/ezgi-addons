<!---
    File: add_ezgi_ship_dispatch.cfm
    Folder: Add_Ons\ezgi\e_shipping\query
    Author: Ezgi Yazılım
    Date: 01/10/2025
    Description: Hızlı Sevk İrsaliyesi Oluşturma İşlemi
--->
...
<cfquery name="get_process_cat" datasource="#DSN3#">
	...
	WHERE     
    	SPC.PROCESS_TYPE = 81 AND 
        SPCF.FUSE_NAME = 'stock.add_ship_dispatch' AND 
	SPC.IS_DEFAULT = 1
  	ORDER BY
...
<cfquery name="get_internaldemand_info" datasource="#dsn3#">
	...
	WHERE  
    	ESR.SHIP_RESULT_INTERNALDEMAND_ID = #attributes.ship_result_id#
</cfquery>
    	...
    	(
        	SELECT 
         	...
         	WHERE  
            	...
            	ESRR.SHIP_RESULT_INTERNALDEMAND_ID = #attributes.ship_result_id# AND 
        ...
        (
        	SELECT 
          	...
          	FROM
                ...
                WHERE 
                	ISNULL(S.IS_ZERO_STOCK,0)=0 AND   
                    STL.DEPO = '#get_internaldemand_info.DEPO#' AND
                    ISNULL(S.IS_PROTOTYPE, 0) = 1
                UNION ALL
                SELECT 
                    STL.PRODUCT_STOCK, 
...
<cfif get_internaldemand_info.recordcount>
<cftransaction>
    ...
    <cfset attributes.ROWS_ = get_internaldemand_info.recordcount>
    <cfset attributes.DEPARTMENT_ID = get_internaldemand_info.DEPARTMENT_ID>
    <cfset attributes.LOCATION_ID = get_internaldemand_info.LOCATION_ID> 
    <cfset attributes.ACTIVE_PERIOD = session.ep.period_id>
 	<cfset attributes.DEPARTMENT_IN_ID = get_internaldemand_info.DEPARTMENT_IN_ID>
	<cfset attributes.LOCATION_IN_ID = get_internaldemand_info.LOCATION_IN_ID> 
    <cfset attributes.EMPLOYEE_ID = session.ep.userid>
    <cfset attributes.ship_detail = 'Sipariş No : #get_internaldemand_info.ORDER_NUMBER# - Sevk Talep No : #attributes.ship_result_id#'> 
    <cfset attributes.dispatch_ship_id = attributes.ship_result_id>
 	...
 	<cfloop query="get_internaldemand_info">   
    	<cfquery name="get_stock_info" datasource="#dsn2#">
            ...
            WHERE     
                ...
                S.STOCK_ID = #get_internaldemand_info.STOCK_ID#
        </cfquery>
		<cfset k = get_internaldemand_info.currentrow>
        ...
        <cfset 'attributes.AMOUNT#k#' = get_internaldemand_info.QUANTITY>
        <cfset 'attributes.AMOUNT_OTHER#k#' = get_internaldemand_info.QUANTITY>
        ...
        <cfset 'attributes.SPECT_ID#k#' = get_internaldemand_info.SPECT_VAR_ID>  
        <cfset 'attributes.TAX_PRICE#k#' = ''>  
        <cfset 'attributes.UNIT_OTHER#k#' = ''>  
        <cfset 'attributes.WRK_ROW_RELATION_ID#k#' = get_internaldemand_info.WRK_ROW_ID>
        <cfset 'attributes.TAX#k#' = 0>
        <cfset 'attributes.SPECT_NAME#k#' = get_internaldemand_info.SPECT_VAR_NAME>
	...
	<cfinclude template="add_ezgi_ship_dispatch_fis_include.cfm">
</cftransaction>
<script type="text/javascript">
 	alert("Kayıt Tamamlanmıştır. Sizi Fişe Yönlendiriyorum!");
	window.location ="<cfoutput>#request.self#?fuseaction=stock.add_ship_dispatch&event=upd&ship_id=#MAX_ID.IDENTITYCOL#</cfoutput>";
