	...
	<cftransaction>
    	<cfif ListLen(attributes.row_info_id)>
        	<cfloop list="#attributes.row_info_id#" index="kj">
                ...
                <cfquery name="get_packing_control" datasource="#dsn2#"> <!---Palet Çıkış Lokasyonunda mı? (Çiftleme hatası için Kontrol)--->
                	SELECT 
                    	PACKING_ID
					FROM     
                    	#dsn3_alias#.EZGI_WM_PACKING_LAST_STATUS
					WHERE  
                    	PACKING_ID = #ListGetAt(kj,2,'-')# AND 
                        STORE = #ListGetAt(attributes.dep_out,1,'-')# AND 
                        STORE_LOCATION = #ListGetAt(attributes.dep_out,2,'-')# 
                        <cfif isdefined('attributes.shelf_out') and len(attributes.shelf_out)> AND SHELF_NUMBER = #attributes.shelf_out#</cfif>
                </cfquery>
                <cfif not get_packing_control.recordcount>
					<script type="text/javascript">
                        alert("<cfoutput>#ListGetAt(kj,2,'-')#</cfoutput> ID li Palet Yerinde Bulunamadı. Tekrar Kontrol Edin!");
						window.history.go(-1);
                    </script>
                    <cfabort>
                </cfif>
...
<cflocation url="#request.self#?fuseaction=stock.list_ezgi_pallets_to_warehouse&is_submitted=1" addtoken="no">
