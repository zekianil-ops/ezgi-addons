<!---
    File: add_ezgi_pallets_to_warehouse.cfm
    Folder: Add_Ons\ezgi\e_wm\form
    Author: Ezgi Yazılım
    Date: 01/08/2023
    Description: Ambara Transfer
--->
...
<cfparam name="attributes.department_out_id" default="#Replace(get_default_departments.INTERMEDIATE_WAREHOUSE,',','-')#">
<cfset default_departments = '#ListGetAt(get_default_departments.PRODUCTION_WAREHOUSE,1)#,#ListGetAt(get_default_departments.SHELF_WAREHOUSE,1)#,#ListGetAt(get_default_departments.INTERMEDIATE_WAREHOUSE,1)#'> <!---Depo seçiminde select satırına gelecek Lokasyonların depatmanları tanımlanır--->
...
<cfsavecontent variable="title_"><cf_get_lang dictionary_id='1307.Ambara Transfer İşlemi'></cfsavecontent>
<cf_catalystHeader>
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
	<cf_box title="#title_#">
        <cfform name="add_ezgi_pallets_to_warehouse" >
            ...
            <cf_basket_form id="add_ezgi_pallets_to_warehouse">
                <div class="row">
...
<script language="javascript" type="text/javascript">
	...
	function get_stock(barcode) /*Ürün Kontrolü*/
    {
		...
		/*var new_sql = "SELECT PACKING_ID, PACKING_SIZE_TYPE_CODE, PACKING_SIZE_TYPE_ID, SHELF_SIZE_TYPE_ID FROM EZGI_WM_PACKING_LAST_STATUS WHERE DEPO = '"+document.getElementById('txt_department_out').value+"' AND BARCODE = '"+barcode+"'";*/ /*Palet Depoda Mevcutmu*/
		/*var get_palettes = wrk_query(new_sql,'dsn3');*/
		
		var listParam = document.getElementById('txt_department_out').value + "*" + barcode;
		var get_palettes = wrk_safe_query('get_packing_wm_depocode_packingcode_ezgi','dsn3',0,listParam);
		...
		if(row_count >0) /*ilk Satırdan sonrası*/
	  	{
			...
			if(sor==true)
			{
				window.location.href='<cfoutput>#request.self#?fuseaction=stock.emptypopup_add_ezgi_pallets_to_warehouse&process_cat=#get_process_cat.process_cat_id#&packing_action_type_id=#PACKING_ACTION_TYPE_ID#</cfoutput>&dep_in='+document.getElementById('txt_department_in').value+'&dep_out='+document.getElementById('txt_department_out').value+'&shelf_in='+document.getElementById('txt_shelf_in').value+'&row_info_id='+document.getElementById('row_info_id').value;	
			}
			else
			{
