...
<div class="col col-12 col-md-12 col-sm-12 col-xs-12">
	<cf_box>
        <cfform name="add_ezgi_pallets_to_storage_shelf_warehouse" >
            ...
            <cfinput id="palet_is_karma" type="hidden" name="palet_is_karma" value="2">
...
<script language="javascript" type="text/javascript">
	...
	function get_stock(barcode) /*Ürün Kontrolü*/
    {
		...
		/*var new_sql = "SELECT PACKING_ID, SHELF_NUMBER, DEPO, PACKING_SIZE_TYPE_CODE, PACKING_SIZE_TYPE_ID, SHELF_SIZE_TYPE_ID, ISNULL(IS_KARMA,0) IS_KARMA FROM EZGI_WM_PACKING_LAST_STATUS WHERE BARCODE = '"+barcode+"'";*/ /*Palet Nerede*/
		...
		{
			if(get_palettes.IS_KARMA==1)
				shelf_type = 3;
			else
				shelf_type = 2;
			document.getElementById('palet_is_karma').value = shelf_type; 
			...
			{
				/*var empty_shelf_sql = "SELECT TOP (1) PP.PRODUCT_PLACE_ID, PP.SHELF_CODE FROM PRODUCT_PLACE AS PP INNER JOIN EZGI_WM_DEPARTMENTS AS E ON PP.STORE_ID = E.DEPARTMENT_ID AND PP.LOCATION_ID = E.LOCATION_ID LEFT OUTER JOIN EZGI_WM_PACKING_LAST_STATUS AS EPL ON PP.PRODUCT_PLACE_ID = EPL.SHELF_NUMBER WHERE PP.SHELF_TYPE = "+shelf_type+" AND E.DEPO = '"+get_palettes.DEPO+"' AND PP.PLACE_STATUS = 1 AND PP.SHELF_SIZE_TYPE = (SELECT SHELF_SIZE_TYPE_ID FROM EZGI_WM_PACKING_LAST_STATUS WHERE EPL.SHELF_NUMBER IS NULL AND PACKING_ID = "+get_palettes.PACKING_ID+") ORDER BY ISNULL(PP.COLLECT_SORT,9999999), ISNULL(PP.SHELF_SORT,9999999)";*/
				/*var get_empty_shelf = wrk_query(empty_shelf_sql,'dsn3');*/
				
				var listParam = shelf_type + "*" + get_palettes.DEPO + "*" + get_palettes.PACKING_ID;
				var get_empty_shelf = wrk_safe_query('get_shelf_wm_shelftype_depocode_packingid_ezgi','dsn3',0,listParam);
				
				if (get_empty_shelf.SHELF_CODE == undefined) /*Palet Bulunamadıysa*/
				{
				...
				else
				{
					...
					if(row_count==0)/*İlk Palet İse*/
					{
						...
						document.getElementById('txt_shelf_in_empty').value=get_empty_shelf.SHELF_CODE;
	...
	{
		/*var shelves_sql = "SELECT TOP (1) PP.PRODUCT_PLACE_ID, PP.SHELF_CODE, PP.PLACE_STATUS, PP.STORE_ID, PP.LOCATION_ID, PP.SHELF_TYPE, E.DEPO_NAME, E.DEPO, PP.SHELF_SIZE_TYPE, EPL.SHELF_NUMBER, EPL.BARCODE, ISNULL(EPL.IS_KARMA,0) AS IS_KARMA FROM PRODUCT_PLACE AS PP INNER JOIN EZGI_WM_DEPARTMENTS AS E ON PP.STORE_ID = E.DEPARTMENT_ID AND PP.LOCATION_ID = E.LOCATION_ID LEFT OUTER JOIN EZGI_WM_PACKING_LAST_STATUS AS EPL ON PP.PRODUCT_PLACE_ID = EPL.SHELF_NUMBER WHERE PP.SHELF_CODE = '"+shelf_in_name+"'";*/ /*Palet İçin Raf Doğrulama*/
		...
		shelf_type = document.getElementById('palet_is_karma').value;
		...
		{
			if(get_shelves.DEPO != document.getElementById('txt_department_out').value)
			{
				alert(get_shelves.DEPO);
				alert(document.getElementById('txt_department_out').value);
				alert('Farklı Depo Kullanamazsınız');
				document.getElementById('txt_shelf_in_name').value = ''; /*Farklı Depo ise Giriş Rafına Odaklan*/
				document.getElementById('txt_shelf_in_name').focus();	
				return false;
			}
			else
			{
				if(get_shelves.SHELF_TYPE != shelf_type)
				{
					if(shelf_type==3)
						alert(shelf_in_name+' Karma Raf Değildir.');
					else if(shelf_type==2)
						alert(shelf_in_name+' Stoklama Rafı Değildir.');
				...
				else
				{
					...
					if(get_shelves.SHELF_SIZE_TYPE != document.getElementById('SIZE_ID1').value <cfif x_is_karma_palet_size_control eq 1> && shelf_type == 2</cfif>) <!---x_is_karma_palet_size_control =0 ise palet-Raf Büyüklük Kontrolü Tüm Tiplerde Yapılır, x_is_karma_palet_size_control=1 ise Sadece Standart Palet-Raflarda Yapar, Karma Palet-Raflarda Kontrol Yapmaz--->
					{
						alert(shelf_in_name+' Raf Büyüklüğü Uygun Değildir. Lütfen Tavsiye Edilen Rafı Tercih Edin');
						document.getElementById('txt_shelf_in_name').value = ''; /*Giriş Rafı Değilse ise Giriş Rafına Odaklan*/
						document.getElementById('txt_shelf_in_name').focus();	
						return false;
					}
					...
					{
						if(get_shelves.BARCODE !='' && shelf_type==2)
						{
							alert(shelf_in_name+' - Rafı Doludur. Lütfen Tavsiye Edilen Rafı Tercih Edin');
							document.getElementById('txt_shelf_in_name').value = ''; /*Giriş Rafı Değilse ise Giriş Rafına Odaklan*/
							document.getElementById('txt_shelf_in_name').focus();	
							return false;
						}
