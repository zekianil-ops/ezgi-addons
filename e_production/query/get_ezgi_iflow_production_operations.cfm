...
<cfelse> <!---parti Seöeneği Evet İse--->
	...
	<cfquery name="get_production_operations" datasource="#dsn3#">
		SELECT 
            ...
            S.PRODUCT_CODE, 
            S.PRODUCT_NAME,
            ROUND(AMT.URETILEN, 2) AS URETILEN
        ...
        FROM     
        	...
        	STOCKS AS S  WITH (NOLOCK) ON EPO.STOCK_ID = S.STOCK_ID LEFT JOIN 
            (
    			SELECT 
                    LOT_NO, 
                ...
                GROUP BY 
                LOT_NO
			) AS AMT ON AMT.LOT_NO = EPO.LOT_NO
    ...
    <cfif ListLen(iflow_p_order_id_list)>
    	<cfif attributes.x_station_level eq 1>	<!---İstasyon Bazında--->
                    ...
                    (	
						...
						FROM    
                        	EZGI_IFLOW_PRODUCTION_ORDERS AS EPO WITH (NOLOCK) INNER JOIN
                  			PRODUCTION_ORDERS AS PO WITH (NOLOCK) ON EPO.LOT_NO = PO.LOT_NO INNER JOIN
                  			PRODUCTION_OPERATION AS PORR WITH (NOLOCK) ON PO.P_ORDER_ID = PORR.P_ORDER_ID INNER JOIN
                  			WORKSTATIONS_PRODUCTS AS WP ON PORR.OPERATION_TYPE_ID = WP.OPERATION_TYPE_ID INNER JOIN
                  			WORKSTATIONS AS W ON WP.WS_ID = W.STATION_ID INNER JOIN
                  			WORKSTATIONS AS W1 ON W.UP_STATION = W1.STATION_ID
						GROUP BY 
                        	EPO.IFLOW_P_ORDER_ID, 
                            PORR.STAGE, 
                            W1.STATION_ID
            ...
            <cfif get_operation_result.recordcount>
                <cfoutput query="get_operation_result">
                    <cfset 'STAGE_#IFLOW_P_ORDER_ID#_#STATION_ID#' = STAGE>
                </cfoutput>
            </cfif>
    	<cfelse><!---Operasyon Bazında--->
        ...
        <cfif get_station_sablon_main.POINT_METHOD eq 2> <!---Puanlıise--->
		   <!---Operasyon Hesaplama--->
