...
<cfif isdefined('attributes.chng_master_alt_plan') and len(attributes.chng_master_alt_plan)> <!---Gönderilmek İstenilen Alt Plan Seçilip Seçilmediğini Kontrol Ediyoruz--->
	<cfquery name="get_lot_no" datasource="#dsn3#"> <!---Seçilen Emirlere ait Lotları Topluyoruz--->
    	SELECT LOT_NO FROM PRODUCTION_ORDERS WHERE P_ORDER_ID IN (#attributes.p_order_list#)
    </cfquery>
    ...
    <cfquery name="get_chng_related_sub_plans" datasource="#dsn3#">
        ...
        WHERE     
            MASTER_ALT_PLAN_ID = #attributes.chng_master_alt_plan# OR
            RELATED_MASTER_ALT_PLAN_ID = #attributes.chng_master_alt_plan#
    </cfquery>
    <cfdump var="#get_chng_related_sub_plans#">
