    ...
    function get_subs(spect_main_id,production_tree_id,type)
    {
		...
		(
			...
			UNION ALL
				...
				FROM 
					OPERATION_TYPES OP WITH (NOLOCK),
					SPECT_MAIN_ROW SMR WITH (NOLOCK)
				WHERE
		...
		ORDER BY
			...
			PRODUCT_NAME
		";
        query1 = workcube_query(SQLString : SQLStr, Datasource : dsn3);
        stock_id_ary='';
        ...
        for (i=1; i lte listlen(sub_products,'█'); i = i+1)
        {
            ...
            //tanımlamalar
                ...
                if(_next_is_production_ eq 1 and _next_is_phantom_ eq 0)//eğer ürünümüz üretilen bir ürün ise bağlı bulunduğu istasyonu bu ürünün ağacından alıyoruz
                {
					...
					GET_PRODUCT_STATION_ID = workcube_query(SQLString : SQL_PRODUCT_STATION_ID, Datasource : dsn3);
    ...
    <cfloop query="GET_AMOUNT">
        ...
        <cfif not len(_spec_main_id_)>
            <cfquery name="get_product_main_spec_default" datasource="#dsn3#"><!--- Ürün Ağacından En Son Varyasyonlanan yani kaydedilen SPECT_MAIN_ID'yi getiriyor. --->
                SELECT TOP 1 SPECT_MAIN_ID FROM SPECT_MAIN SM WITH (NOLOCK) WHERE SM.STOCK_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#_stock_id_#"> AND SM.IS_TREE = 1 ORDER BY SM.RECORD_DATE DESC,SM.UPDATE_DATE DESC
            </cfquery>
