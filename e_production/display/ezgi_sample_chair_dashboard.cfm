	...
	<cfquery name="get_orders" datasource="#dsn3#">
		SELECT 
            	...
            	WHERE   
                    ...
                    S.PRODUCT_CODE LIKE N'01.152.02.01%' 
	</cfquery>   
                    ...
                    THEN
                        ISNULL((
                            SELECT TOP (1)
                                PORRR.ORDER_ROW_ID
                            FROM     
                                PRODUCTION_ORDERS_ROW AS PORRR WITH (NOLOCK) INNER JOIN
                                ORDER_ROW AS ORR WITH (NOLOCK) ON PORRR.ORDER_ROW_ID = ORR.ORDER_ROW_ID
                            WHERE  
                                PORRR.PRODUCTION_ORDER_ID = PO.P_ORDER_ID
                        ),0)
                    WHEN
                        ISNULL(S.IS_SERIAL_NO,0) = 1
                    THEN
    	...
    	SELECT
       	...
       	FROM
			...
			WHERE  
                ...
                E.LOCATION_ID = 8 AND 
                S.STOCK_CODE LIKE N'01.152.02.01%'
